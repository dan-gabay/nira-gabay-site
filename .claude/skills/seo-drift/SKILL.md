...
2. Fetch page via `".claude/skills/seo/bin/claude-seo" run fetch_page.py <URL>`
3. Parse HTML via `".claude/skills/seo/bin/claude-seo" run parse_html.py <URL>`
4. Optionally fetch CWV via `".claude/skills/seo/bin/claude-seo" run pagespeed_check.py <URL>` (use `--skip-cwv` to skip)
...
- **All URL fetching** goes through `".claude/skills/seo/bin/claude-seo" run fetch_page.py`, which enforces SSRF protection
