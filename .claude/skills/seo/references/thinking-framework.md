...
- Raw HTML + rendered HTML (via `".claude/skills/seo/bin/claude-seo" run render_page.py`)
...
- Core Web Vitals field data from CrUX (via `".claude/skills/seo/bin/claude-seo" run pagespeed_check.py`)
...
- A PDF via `".claude/skills/seo/bin/claude-seo" run google_report.py` when the user asks for one.
