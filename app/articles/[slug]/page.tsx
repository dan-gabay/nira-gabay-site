...
export default async function ArticlePage({ params }: Props) {
  ...
  // Structured Data. One builder for every article, pipeline-generated or not:
  // it reuses the stored schema_json's editorial fields and recomputes the
  // rest - author, publisher, dateModified, url - from the live row, so the
  // author is a reference to the site's Person node instead of a fourth copy
  // of her name. See lib/articleSchema.ts.
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-stone-50" style={{ paddingTop: '80px' }}>
      ...
      {/* Header. A <header>, not a <section>: it carries the H1, the byline and
          the dates, which is what a reader-mode or an extraction pass looks for
          to separate an article's masthead from its body. Same element box, so
          the layout is byte-identical. */}
      <header className="py-4 md:py-6 bg-gradient-to-br from-stone-100 to-amber-50">
        <div className="container mx-auto px-4 md:px-8 max-w-4xl">
          {/* Tags */}
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 md:gap-4 text-xs md:text-sm text-stone-500">
            {/* rel="author" plus a link to the author page: the machine-readable
                half of a byline. Renders identically - the anchor inherits the
                same colour and weight the span had. */}
            <Link href="/about" rel="author" className="flex items-center gap-2 hover:text-stone-800">
            ...
            {article.created_date && (
              <time
                dateTime={new Date(article.created_date).toISOString()}
