...
| P1-4 | ~~Visible FAQ accordion on article pages~~ **DONE 2026-07-07** | ArticleFaq renders faq.mainEntity as a no-JS `<details>` accordion above the author box, 1:1 with the FAQPage JSON-LD. Bonus: article breadcrumb (visible + schema) now includes the primary tag's hub level. Verified live |
| P1-5 | ~~Homepage "latest articles" section~~ **DONE 2026-07-07** | Section already existed but fetched client-side (useEffect) - links were invisible to crawlers. Now fetched in app/page.tsx (ISR 5 min) and passed as props; links verified in initial HTML |
| P1-6 | ~~Recalibrate validator~~ **DONE 2026-07-07** | validate.ts: generic 1-word focus keywords (curated list) = error; missing keyword = error; <300 words = error, <600 = warn (YMYL); body-H1 downgraded to warn (renderer demotes to H2, verified live). scripts/rescore-published.ts re-scored all 26 from STORED fields: honest range now 68-92. Real work queue = 4 articles with errors: depression-cycle (null schema_json), talking-to-children/passover/back-to-school (generic keywords) |
| P1-7 | ~~dateModified fixes~~ **DONE 2026-07-07** | Article page: og modifiedTime added; BlogPosting dateModified always overridden with live updated_date (stored schema_json froze it at insert) |
| P1-8 | ~~Birth-order consolidation~~ **DONE 2026-07-07 (owner-approved)** | birth-order-family-dynamics unpublished (status='redirected'), permanent 301 to family-birth-order-meaning-and-impact in next.config.ts. CLAUDE.md protection note updated. Internal links regenerated (28 articles, slug fully removed). Redirected/superseded rows excluded from pipeline checks |
