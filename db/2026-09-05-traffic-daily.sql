-- Adds one series to manage_analytics: visits per day, split by traffic source
-- group - the same six groups the "מאיפה הגיעו" card already buckets sessions
-- into, so the card and the new graph can never disagree about what "ממומן"
-- means.
--
-- Two edits, both small, to a function that is otherwise reproduced verbatim
-- from the live definition so nothing else can drift:
--
--   1. `sess` gains `started_at` - the session's first event. A session is
--      attributed to one source for its whole life (that is what the existing
--      first-non-null aggregation does), so it belongs to exactly one day: the
--      day it began. Bucketing by each event's own timestamp would spread one
--      visit across two days whenever someone reads past midnight.
--
--   2. A `traffic_daily` key, bucketed on the same hour/day boundary as the
--      existing `daily` series, so the two charts share an x-axis.
--
-- Only groups with at least one session in the range appear, which is how the
-- existing card behaves too - a source that sent nobody is absent rather than
-- being drawn as a flat zero line.
...
as $$
  ...
  select json_build_object(
    ...
    'traffic_daily', (select coalesce(json_agg(t order by t.day, t.grp), '[]'::json) from (
        select to_char(
                 date_trunc(case when p_days <= 1 then 'hour' else 'day' end, started_at),
                 case when p_days <= 1 then 'YYYY-MM-DD"T"HH24' else 'YYYY-MM-DD' end
               ) as day,
               grp,
               count(*)                          as visits,
